...
export const watch = async (glueStackPlugin: GlueStackPlugin) => {
  let deploymentId = glueStackPlugin.gluePluginStore.get("deployment_id");
  ...
  let interval = setInterval(async () => {
    const isDone: boolean = await getDeployment(deploymentId, glueStackPlugin);
    if (isDone) {
...
async function getDeployment(
  ...
  try {
    ...
    const { db_deployments_by_pk } = await deployment(deploymentId, team.token);
